...
| **모바일 단말 앱** | 승조원/승객 개인별 대피 경로 안내 | iOS/Android, 실시간 위치추적 연동 |
...
##### 3.1.5 SOLAS/IMO 기반 10종 재난 시나리오 상세 정의

I-SEDSS는 국제해사기구(IMO)의 **MSC.1/Circ.1533** 및 **SOLAS** 표준 운영 절차(SOP)를 기반으로 선박에서 발생 가능한 10가지 핵심 재난 시나리오를 정의합니다. 특히 사용자가 요청한 **화재, 침수, 충돌** 상황에 대해서는 센서 데이터와 연동된 정밀 시나리오를 구축합니다.

| 번호 | 시나리오 구분 | 주요 내용 및 환경 변수 | 관련 근거 및 출처 |
|:---:|:---|:---|:---|
| **S1** | **주간 공용구역 화재** | 식당, 라운지 등 다수 인원 밀집 구역 화재 (Day Case) | IMO MSC.1/Circ.1533 |
| **S2** | **야간 객실구역 화재** | 취침 시간대 복도 및 객실 내 화재 발생 (Night Case) | IMO MSC.1/Circ.1533 |
| **S3** | **기관실 대형 화재** | 메인 엔진 화재로 인한 비상 발전기 가동 및 정전 상황 | SOLAS II-2 Reg. 10 |
| **S4** | **주간 선체 충돌/침수** | 타선 충돌로 인한 좌현 수밀 격벽 파손 및 침수 발생 | IMO MSC.1/Circ.1533 |
| **S5** | **야간 선체 충돌/침수** | 야간 항해 중 우현 충돌에 따른 구획별 순차적 침수 | IMO MSC.1/Circ.1533 |
| **S6** | **선박 좌초 및 경사** | 암초 충돌로 인한 선체 하부 파손 및 급격한 경사(Heel) 발생 | SOLAS Ch.II-1 |
| **S7** | **태풍 및 악천후** | 강풍 및 파랑에 따른 비정상적 롤링 및 외부 갑판 통제 | ISM Code SOP |
| **S8** | **주방/조리실 화재** | 조리 시 유류 화재 발생 및 덕트를 통한 타 구역 확산 | SOLAS II-2 Reg. 9 |
| **S9** | **대규모 유류 유출** | 연료 탱크 파손으로 인한 유출 및 유증기 폭발 위험 상황 | MARPOL Annex I |
| **S10** | **복합 재난 (최악 조건)** | 충돌 후 화재 및 침수 동시 발생 및 메인 전력 차단 상황 | IMO/SOLAS 통합 대응 |

---

###### [집중 고도화 시나리오 상세]

**1. 화재 및 화재 확산 (Fire & Spread) - S1, S2, S3**
*   **사고 인식**: AI 멀티모달 센서(LiDAR + 열화상)를 통해 화염의 파장과 연기 확산 속도를 초당 분석합니다. 단순 연무(Cigarette, Cooking)와 실제 화재를 99% 정확도로 구분합니다.
*   **대응 체계**: 고온 및 농연 발생 구역을 '차단 구역'으로 선포하고, FDS(Fire Dynamics Simulator) 데이터를 기반으로 5분 후 연기 도달 예상 경로를 미리 회피하는 동적 유도를 실시합니다.
*   **출처**: IMO MSC.1/Circ.1533 (Guidelines for Evacuation Analysis)

**2. 선박 침수 (Flooding) - S4, S5**
*   **사고 인식**: 선저 및 수밀 격벽에 설치된 침수 센서(Level Sensor)의 연속 신호를 감지합니다. 이와 동시에 선체 경사계(Inclinometer)의 데이터가 특정 임계값(예: 7도 이상)을 초과할 경우 재난을 확정합니다.
*   **대응 체계**: 침수 속도와 복원성(Stability) 변화를 계산하여 '최후 탈출 시점(Abandon Ship Time)'을 역산합니다. 침수로 인해 통행이 불가능해진 하층부 계단을 대피 경로에서 실시간 제거합니다.
*   **출처**: SOLAS Chapter II-1 (Construction - Subdivision and stability)

**3. 선박 충돌 (Collision) - S4, S10**
*   **사고 인식**: AIS/RADAR 데이터상의 근접 경보 후, 선체에 가해지는 물리적 충격 데이터(G-Sensor)와 소음 데이터를 분석하여 충돌 위치와 파손 규모를 1차 판단합니다.
*   **대응 체계**: 충돌 지점을 중심으로 인접 구역의 수밀문을 강제 폐쇄 지시하며, 충격으로 인해 단절된 네트워크 구간을 메시(Mesh) 네트워크로 긴급 복구하여 고립자를 파악합니다.
*   **출처**: ISM Code 및 IMO Safety Management Manual
...
flowchart LR
    subgraph Y1["📅 1차년도 수행 내용"]
        ...
        subgraph Q3Q4["Q3-Q4: AI 개발"]
            ...
            T4["🖥️ 시뮬레이션 환경<br/>━━━━━━━━━━<br/>• 3D 디지털 트윈 구축<br/>• FDS 연동 인터페이스<br/>• 실시간 시각화 프로토타입"]
        end
    end
...
| 세부 과제 | 수행 내용 | 연구 방법 | 담당 기관 | 산출물 |
|:---|:---|:---|:---|:---|
| **디지털 트윈 개발** | 새동백호 3D 모델 기반 가상 환경 | 3D 시뮬레이션 시각화 | 전남대 | 디지털 트윈 v1.0 |
| **FDS 연동** | 실시간 화재 확산 시각화 | FDS 연동 모듈 개발 | 전남대 | FDS 연동 모듈 |
...
flowchart LR
    subgraph Y2["📅 2차년도 수행 내용"]
        ...
        subgraph Q3Q4["Q3-Q4: 플랫폼 개발"]
            ...
            T4["📱 모바일 앱 개발<br/>━━━━━━━━━━<br/>• 승조원용 앱 개발<br/>• 승객용 앱 개발<br/>• 실시간 위치추적 기술 연동"]
        end
    end
...
| **시뮬레이션** | 디지털 트윈 연동 성능 검증 | 3D 시뮬레이션 엔진 |
