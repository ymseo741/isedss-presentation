...
|:---|:---|:---|:---|:---|
| **재난 시나리오 정의** | 선박 화재 유형별 시나리오 10종 설계 | IMO MSC.1/Circ.1238 기반 분석 | 전남대 | 시나리오 명세서 |
...
1. **시나리오 기반 설계 (Scenario-Based Design)**
   - IMO 가이드라인에서 정의한 선박 화재 유형 분류 체계 활용
   - 화재 발생 위치: 기관실, 주방, 객실, 차량 구역 등 5개 구역
   - 화재 규모: 소규모(초기 진화 가능), 중규모(구역 확산), 대규모(다층 확산)
   - 시간대: 주간/야간, 운항 중/정박 중 시나리오 분리
...
| 세부 과제 | 수행 내용 | 연구 방법 | 담당 기관 | 산출물 |
|:---|:---|:---|:---|:---|
| **화재 감지 모델** | 멀티모달 센서 퓨전 기반 화재 판정 | CNN + Attention 아키텍처 | 한빛안전기술 | 화재 감지 모델 v1.0 |
...
| **모델 통합 파이프라인** | E2E 추론 파이프라인 구축 | PyTorch + ONNX 최적화 | 오든 | 통합 추론 엔진 v1.0 |
...
| **테스트 프레임워크** | 자동화된 시나리오 테스트 환경 | pytest + CI/CD | 오든 | 테스트 프레임워크 |
...
| 세부 과제 | 수행 내용 | 연구 방법 | 담당 기관 | 산출물 |
|:---|:---|:---|:---|:---|
| **실시간 재탐색 알고리즘** | 화재 확산에 따른 동적 경로 재계산 | Incremental GNN + A* | 한빛안전기술 | 재탐색 엔진 v2.0 |
| **Edge 최적화** | Jetson AGX 배포용 경량화 | TensorRT + Quantization | 한빛안전기술 | Edge 최적화 모델 |
| **지연시간 최적화** | 3초 이내 응답 보장 | 캐싱 + 병렬처리 | 오든 | 성능 최적화 보고서 |
| **장애 복구 로직** | 센서/네트워크 장애 시 대응 | Fallback 알고리즘 | 오든 | 장애 복구 모듈 |

**실시간 재탐색 알고리즘 상세:**

```
재탐색 트리거 조건:
├── 화재 위치 변경: FDS 예측 결과 갱신 시
...
| 세부 과제 | 수행 내용 | 연구 방법 | 담당 기관 | 산출물 |
|:---|:---|:---|:---|:---|
| **3D 관제 UI** | 실시간 선박 상황 3D 시각화 | React + Three.js + WebGL | 오든 | 관제 UI v1.0 |
| **상황 대시보드** | 센서 상태, 알림, 대피 현황 | React + D3.js | 오든 | 대시보드 v1.0 |
| **XAI 인터페이스** | AI 판단 근거 시각화 | SHAP + Custom UI | 한빛안전기술 | XAI UI v1.0 |
| **API 게이트웨이** | 모듈 간 통신 허브 | FastAPI + Kong | 오든 | API Gateway v1.0 |

##### 5.2.3 모바일 앱 및 유도 시스템 개발 (Q3-Q4)

| 세부 과제 | 수행 내용 | 연구 방법 | 담당 기관 | 산출물 |
|:---|:---|:---|:---|:---|
| **승조원 앱** | 상세 상황 정보, 지시 수신 | React Native + UWB | 오든 | 승조원 앱 v1.0 |
| **승객 앱** | 간단한 대피 경로 안내 | React Native | 오든 | 승객 앱 v1.0 |
| **동적 LED 유도등 프로토콜** | 실시간 방향 제어 프로토콜 | RS-485 + MQTT | 한빛안전기술 | LED 제어 프로토콜 |
| **UWB 위치추적** | 실내 위치 추적 정확도 검증 | Decawave SDK | 한빛안전기술 | 위치추적 모듈 v1.0 |
...
| **기존 시스템 연동** | VMS, 방송 시스템 연동 | API 연동 개발 | 오든 | 연동 테스트 완료 |
...
| **대피 훈련 1차** | 야간 화재 시나리오 | 실제 승조원 참여 | 해양수산과학기술연구조합 | 1차 훈련 결과 |
| **대피 훈련 2차** | 주간 다중 화재 시나리오 | 승객 참여 (자원자) | 해양수산과학기술연구조합 | 2차 훈련 결과 |
| **대피 훈련 3차** | 복합 재난 시나리오 | 전체 훈련 | 해양수산과학기술연구조합 | 3차 훈련 결과 |
| **성능 데이터 수집** | KPI 달성 여부 측정 | 자동 로깅 + 수동 측정 | 오든 | 성능 측정 보고서 |

**대피 훈련 상세 계획:**

```
훈련 1차 (M28): 야간 화재 대응
├── 시나리오: 기관실 화재, 야간 23:00
├── 참여: 승조원 전원 (약 50명)
├── 목표: 시스템 알림 → 대피 시작 2분 이내
└── 측정: 감지 시간, 경로 정확도, 대피 완료 시간

훈련 2차 (M30): 주간 다중 화재
├── 시나리오: 주방 + 차량 구역 동시 화재, 주간 14:00
├── 참여: 승조원 + 승객 자원자 (약 200명)
├── 목표: 동적 경로 재탐색 검증
└── 측정: 재탐색 응답 시간, 혼잡 구역 회피율

훈련 3차 (M32): 복합 재난
├── 시나리오: 화재 + 정전 + 통신 두절, 야간 03:00
├── 참여: 전체 승객 (최대 수용 시)
...
| 시점 | 마일스톤 | 달성 기준 | 검증 방법 |
|:---:|:---|:---|:---|
| M27 | 하드웨어 설치 완료 | 전 센서/장비 정상 동작 | 현장 검수 |
...
| **UWB 앵커** | Decawave DW1000 또는 동급 | ±10cm 정확도, 50m 범위 | 대피 경로 | 금속 환경 성능 테스트 |
...
| 구역 | LiDAR | 열화상 | 연기감지기 | CCTV | UWB앵커 | Edge Node |
|:---|:---:|:---:|:---:|:---:|:---:|:---:|
| 브릿지 | - | - | 2 | 2 | - | 1 (Master) |
| 객실 구역 | - | - | 120 | 24 | 30 | 4 |
| 공용 구역 | 4 | 2 | 15 | 12 | 10 | 2 |
| 기관실 | 2 | 4 | 20 | 6 | - | 2 |
| 차량 구역 | - | 6 | 30 | 8 | - | 2 |
| **합계** | **6** | **12** | **187** | **52** | **40** | **11** |
...
Edge Node는 각 구역의 센서 데이터를 실시간으로 수집·처리하는 핵심 컴퓨팅 유닛입니다. AI 추론이 가능한 GPU를 탑재하여 로컬에서 즉시 화재 판정을 수행합니다.
...
| **필수 기능** | UWB, BLE 5.0, GPS | UWB, BLE 5.0, GPS |
...
|:---:|:---|:---|:---|:---:|
| 1 | **센서 제조사 선정** | LiDAR, 열화상 등 주요 센서 업체 확정 | 한빛안전기술 | 2026 Q1 |
| 2 | **Edge Node H/W 선정** | GPU 탑재 산업용 PC 모델 확정 | 오든 | 2026 Q1 |
| 3 | **MQTT Broker 선정** | 오픈소스 vs 상용 라이선스 결정 | 오든 | 2026 Q1 |
| 4 | **데이터 스키마 표준화** | 센서 데이터 JSON Schema 확정 | 전남대 | 2026 Q2 |
| 5 | **API 인터페이스 정의** | 모듈 간 API 명세서 작성 | 오든 + 한빛 | 2026 Q2 |
| 6 | **FDS 연동 방식** | PyroSim 데이터 포맷 및 주기 | 전남대 | 2026 Q2 |
| 7 | **선급 인증 우선순위** | KR, DNV, Lloyd's 중 1차 타겟 | KCL | 2026 Q2 |
...
| 순번 | 항목 | 설명 | 담당 기관 | 결정 시한 |
|:---:|:---|:---|:---|:---:|
| 1 | **개인정보 처리** | 승객 위치 추적 데이터 처리 방침 | 오든 (법무) | 2026 Q1 |
| 2 | **CCTV 영상 저장** | 저장 기간, 접근 권한, 삭제 정책 | 오든 (법무) | 2026 Q1 |
| 3 | **해외 수출 규제** | AI 기술 수출 관련 규정 확인 | 오든 (법무) | 2026 Q3 |
| 4 | **책임 범위 정의** | AI 오판단 시 법적 책임 소재 | 전체 컨소시엄 | 2026 Q2 |
